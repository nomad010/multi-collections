...
impl<T: Eq + Hash, S: BuildHasher> MultiHashSet<T, S> {
    ...
    /// }
    /// ```
    pub fn sum<'a, S2: 'a + BuildHasher>(&'a self, other: &'a MultiHashSet<T, S2>) -> Sum<'a, T, S, S2> {
        Sum::new(self, other)
    }
    ...
    /// }
    /// ```
    pub fn difference<'a, S2: 'a + BuildHasher>(&'a self, other: &'a MultiHashSet<T, S2>) -> Difference<'a, T, S, S2> {
    ...
    /// }
    /// ```
    pub fn min<'a, S2: 'a + BuildHasher>(&'a self, other: &'a MultiHashSet<T, S2>) -> Min<'a, T, S, S2> {
        Min::new(self, other)
    }
    ...
    /// }
    /// ```
    pub fn max<'a, S2: 'a + BuildHasher>(&'a self, other: &'a MultiHashSet<T, S2>) -> Max<'a, T, S, S2> {
        Max::new(self, other)
    }
...
/// A lazy iterator producing elements in the sum of two `MultiHashSet`s.
///
/// This `struct` is created by the [`sum`] method on [`MultiHashSet`].
/// See its documentation for more.
///
/// [`MultiHashSet`]: struct.MultiHashSet.html
/// [`sum`]: struct.MultiHashSet.html#method.sum
#[derive(Clone, Debug)]
pub struct Sum<'a, T: 'a + Hash + Eq, S1: 'a + BuildHasher, S2: 'a + BuildHasher> {
...
impl<'a, T: 'a + Hash + Eq, S1: 'a + BuildHasher, S2: 'a + BuildHasher> Sum<'a, T, S1, S2> {
    pub fn new(self_set: &'a MultiHashSet<T, S1>, other_set: &'a MultiHashSet<T, S2>) -> Sum<'a, T, S1, S2> {
        Sum {
...
impl<'a, T: 'a + Hash + Eq, S1: 'a + BuildHasher, S2: 'a + BuildHasher> Iterator for Sum<'a, T, S1, S2> {
    type Item = (&'a T, usize);
...
impl<'a, T: 'a + Hash + Eq, S1: 'a + BuildHasher, S2: 'a + BuildHasher> FusedIterator for Sum<'a, T, S1, S2> {
}

/// A lazy iterator producing elements in the difference of two `MultiHashSet`s.
///
/// This `struct` is created by the [`difference`] method on [`MultiHashSet`].
/// See its documentation for more.
///
/// [`MultiHashSet`]: struct.MultiHashSet.html
/// [`difference`]: struct.MultiHashSet.html#method.difference
#[derive(Clone, Debug)]
pub struct Difference<'a, T: 'a + Hash + Eq, S1: 'a + BuildHasher, S2: 'a + BuildHasher> {
    self_iterator: HashMapIter<'a, T, usize>,
    other_set: &'a MultiHashSet<T, S2>,
    self_hash_type: PhantomData<S1>
}

impl<'a, T: 'a + Hash + Eq, S1: 'a + BuildHasher, S2: 'a + BuildHasher> Difference<'a, T, S1, S2> {
    pub fn new(self_set: &'a MultiHashSet<T, S1>, other_set: &'a MultiHashSet<T, S2>) -> Difference<'a, T, S1, S2> {
        Difference {
            self_iterator: self_set.iter(),
            other_set: other_set,
...
impl<'a, T: 'a + Hash + Eq, S1: 'a + BuildHasher, S2: 'a + BuildHasher> Iterator for Difference<'a, T, S1, S2> {
    type Item = (&'a T, usize);
...
impl<'a, T: 'a + Hash + Eq, S1: 'a + BuildHasher, S2: 'a + BuildHasher> FusedIterator for Difference<'a, T, S1, S2> {
}

/// A lazy iterator producing elements in the max of two `MultiHashSet`s.
///
/// This `struct` is created by the [`maximum`] method on [`MultiHashSet`].
/// See its documentation for more.
///
/// [`MultiHashSet`]: struct.MultiHashSet.html
/// [`max`]: struct.MultiHashSet.html#method.max
#[derive(Clone, Debug)]
pub struct Max<'a, T: 'a + Hash + Eq, S1: 'a + BuildHasher, S2: 'a + BuildHasher> {
    self_iterator: HashMapIter<'a, T, usize>,
    other_iterator: HashMapIter<'a, T, usize>,
    self_set: &'a MultiHashSet<T, S1>,
    other_set: &'a MultiHashSet<T, S2>,
...
impl<'a, T: 'a + Hash + Eq, S1: 'a + BuildHasher, S2: 'a + BuildHasher> Max<'a, T, S1, S2> {
    pub fn new(self_set: &'a MultiHashSet<T, S1>, other_set: &'a MultiHashSet<T, S2>) -> Max<'a, T, S1, S2> {
        Max {
            self_iterator: self_set.iter(),
            other_iterator: other_set.iter(),
            self_set: self_set,
...
impl<'a, T: 'a + Hash + Eq, S1: 'a + BuildHasher, S2: 'a + BuildHasher> Iterator for Max<'a, T, S1, S2> {
    type Item = (&'a T, usize);
...
impl<'a, T: 'a + Hash + Eq, S1: 'a + BuildHasher, S2: 'a + BuildHasher> FusedIterator for Max<'a, T, S1, S2> {
}

/// A lazy iterator producing elements in the mimimum of two `MultiHashSet`s.
///
/// This `struct` is created by the [`min`] method on [`MultiHashSet`].
/// See its documentation for more.
///
/// [`MultiHashSet`]: struct.MultiHashSet.html
/// [`min`]: struct.MultiHashSet.html#method.min
#[derive(Clone, Debug)]
pub struct Min<'a, T: 'a + Hash + Eq, S1: 'a + BuildHasher, S2: 'a + BuildHasher> {
    self_iterator: HashMapIter<'a, T, usize>,
    other_set: &'a MultiHashSet<T, S2>,
    self_hash_type: PhantomData<S1>,
}

impl<'a, T: 'a + Hash + Eq, S1: 'a + BuildHasher, S2: 'a + BuildHasher> Min<'a, T, S1, S2> {
    pub fn new(self_set: &'a MultiHashSet<T, S1>, other_set: &'a MultiHashSet<T, S2>) -> Min<'a, T, S1, S2> {
        Min {
            self_iterator: self_set.iter(),
            other_set: other_set,
            self_hash_type: PhantomData
...
impl<'a, T: 'a + Hash + Eq, S1: 'a + BuildHasher, S2: 'a + BuildHasher> Iterator for Min<'a, T, S1, S2> {
    type Item = (&'a T, usize);
...
impl<'a, T: 'a + Hash + Eq, S1: 'a + BuildHasher, S2: 'a + BuildHasher> FusedIterator for Min<'a, T, S1, S2> {
}


#[derive(Clone, Debug)]
pub struct Items<'a, T: 'a + Hash + Eq> {
...
impl<'a, T: 'a + Hash + Eq, > Items<'a, T> {
    pub fn new<S: 'a + BuildHasher>(self_set: &'a MultiHashSet<T, S>) -> Items<'a, T> {
        Items {
...
impl<'a, T: 'a + Hash + Eq> Iterator for Items<'a, T> {
    type Item = &'a T;
...
impl<'a, T: 'a + Hash + Eq> FusedIterator for Items<'a, T> {
}
